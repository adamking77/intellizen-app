...
// Post-Phase-9: inbox lives at ingest.voice_notes (was public.voice_notes_inbox)
...
Deno.serve(async (req: Request) => {
  ...
  catch (err) { console.error("invalid JSON body", err); return ok(); }

  const msg = update.message;
  const voice = msg?.voice ?? msg?.audio;
  if (!msg || !voice) return ok();

  try { await handleVoice(msg.chat.id, msg.message_id, voice); }
  catch (err) { console.error("voice handler failed", err); }
  return ok();
});

async function handleVoice(chatId: number, messageId: number, voice: TelegramVoice) {
  const fileInfoRes = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/getFile?file_id=${voice.file_id}`);
...
async function transcribe(audio: Uint8Array, mime: string, ext: string): Promise<string> {
  ...
  if (!res.ok) { const body = await res.text(); throw new Error(`whisper failed (${res.status}): ${body}`); }
  return (await res.text()).trim();
}

async function classify(transcript: string): Promise<{ slug: string; tags: string[] }> {
  const prompt = `Voice note transcript:\n"""\n${transcript}\n"""\n\nReturn ONLY a JSON object with two fields:\n- "slug": a kebab-case identifier (max 6 words) that names the IDEA of the note.\n- "tags": an array of 3-7 kebab-case keywords drawn from the note substance.\n\nReturn only the JSON, no markdown fencing.`;
  ...
  if (!res.ok) { const body = await res.text(); console.error(`haiku failed (${res.status}): ${body}`); return fallbackSlugTags(transcript); }
  ...
  try {
    ...
    const tags = Array.isArray(parsed.tags) ? parsed.tags.map(sanitizeSlug).filter(Boolean).slice(0, 7) : [];
    if (!slug) return fallbackSlugTags(transcript);
    return { slug, tags };
  } catch { console.error(`haiku returned non-JSON: ${text}`); return fallbackSlugTags(transcript); }
}

function sanitizeSlug(raw: unknown): string {
  if (typeof raw !== "string") return "";
  return raw.toLowerCase().replace(/[^a-z0-9\s-]/g, "").trim().replace(/\s+/g, "-").replace(/-+/g, "-").slice(0, 80);
}
