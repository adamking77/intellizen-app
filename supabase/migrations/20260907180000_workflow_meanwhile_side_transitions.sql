-- Persist independent read-only work while an approval cursor remains paused.
...
begin
  ...
  if exists (
    ...
    where state.value = 'completed'
      and not exists (
        select 1 from workspace.workflow_step_result_receipts receipt
        where receipt.workflow_run_id = p_workflow_run_id
          and receipt.execution_version = v_execution_version
          and receipt.definition_hash = v_definition_hash
          and receipt.step_id = state.key
      )
  ) then
...
create or replace function workspace.settle_workflow_side_step(
  p_workflow_run_id uuid,
  p_approval_step_id text,
  p_side_step_id text,
  p_expected_side_step_state text,
...
declare
  ...
  v_execution_version bigint;
  v_definition_hash text;
  v_receipt workspace.workflow_step_result_receipts;
...
begin
  ...
  if p_workflow_run_id is null
    or p_approval_step_id is null or nullif(btrim(p_approval_step_id), '') is null
    or p_side_step_id is null or nullif(btrim(p_side_step_id), '') is null
    or p_expected_side_step_state is distinct from 'running'
    or p_next_side_step_state is null
    or p_next_side_step_state not in ('completed', 'blocked', 'cancelled', 'abandoned')
    ...
    or p_event_summary is null or nullif(btrim(p_event_summary), '') is null
  then
    raise exception 'Workflow side settlement requires assignment, receipt, idempotency, and actor fields';
  end if;
  if jsonb_typeof(coalesce(p_event_payload, '{}'::jsonb)) <> 'object' then
    raise exception 'Workflow event payload must be a JSON object';
  end if;
  if (p_next_side_step_state = 'completed' and p_event_kind is distinct from 'meanwhile_completed')
    or (p_next_side_step_state = 'blocked' and p_event_kind is distinct from 'meanwhile_blocked')
    or (p_next_side_step_state = 'cancelled' and p_event_kind is distinct from 'meanwhile_cancelled')
    or (p_next_side_step_state = 'abandoned' and p_event_kind is distinct from 'meanwhile_abandoned')
  then
  ...
  if nullif(v_run.fields->>'run_dispatcher_session', '') is not null
    and nullif(v_run.fields->>'run_lease_expires_at', '')::timestamptz > clock_timestamp()
  then
    raise exception 'Workflow side settlement waits for active dispatcher lease';
  end if;

  v_states := coalesce(v_run.fields->'run_step_states', '{}'::jsonb);
  if jsonb_typeof(v_states) <> 'object'
    or v_states->>p_side_step_id is distinct from p_expected_side_step_state
  then
    raise exception 'Workflow side step state mismatch: expected %, current %',
      p_expected_side_step_state,
      v_states->>p_side_step_id;
  ...
  from workspace.work_events
  where workflow_run_id = p_workflow_run_id
    and step_id = p_side_step_id
    and assignment_id = p_assignment_id
    and event_kind = 'meanwhile_assignment_created'
    and payload->>'approvalStepId' = p_approval_step_id
    and (payload->>'executionVersion')::bigint =
      coalesce((v_run.fields->>'run_execution_version')::bigint, 0)
    and payload->>'definitionHash' = v_run.fields->>'run_definition_hash'
  order by run_version desc limit 1;
  if not found then
    raise exception 'Workflow side assignment claim was not found';
  end if;
  ...
  begin
    v_execution_version := (v_run.fields->>'run_execution_version')::bigint;
  exception when others then
    raise exception 'Workflow side settlement requires immutable execution identity';
  end;
  v_definition_hash := v_run.fields->>'run_definition_hash';
  perform workspace.workflow_run_execution_identity(
    p_workflow_run_id,
  ...
  v_next_version := v_current_version + 1;
  v_section := format(
    '## %s - %s%s%s%s%s', p_event_kind, clock_timestamp(), E'\n\n',
    format('Actor: %s', p_actor), E'\n', p_event_summary
  );
  update workspace.records
  set fields = jsonb_set(
      jsonb_set(fields, '{run_version}', to_jsonb(v_next_version), true),
      '{run_step_states}', v_states, true
    ),
    body = concat_ws(E'\n\n', nullif(rtrim(body), ''), v_section),
    updated_at = now()
  where id = p_workflow_run_id
  ...
  v_result := jsonb_build_object(
    'applied', true, 'duplicate', false, 'run_version', v_next_version,
    'run', to_jsonb(v_run)
  );
  v_runtime_session_id := nullif(p_event_payload->>'runtimeSessionId', '');
  v_event.id := gen_random_uuid();
  if p_next_side_step_state = 'completed' then
    v_completion_ordinal := (
      select coalesce(max(completion_ordinal), 0) + 1
      from workspace.workflow_step_result_receipts
      where workflow_run_id = p_workflow_run_id
        and execution_version = v_execution_version
        and definition_hash = v_definition_hash
        and step_id = p_side_step_id
    );
    insert into workspace.workflow_step_result_receipts (
      ...
      gen_random_uuid(), p_workflow_run_id, v_execution_version, v_definition_hash,
      p_side_step_id, v_completion_ordinal, v_current_version, v_next_version,
      coalesce(p_event_payload->'result', 'null'::jsonb),
      workspace.jsonb_sha256_v1(coalesce(p_event_payload->'result', 'null'::jsonb)),
      v_event.id, p_actor
    ) returning * into v_receipt;
    v_result := v_result || jsonb_build_object(
      'step_result_receipt', jsonb_build_object(
        'schema', 'intellizen.workflow-step-result/1',
        'receiptId', v_receipt.receipt_id,
        'workflowRunId', v_receipt.workflow_run_id,
        'executionVersion', v_receipt.execution_version,
        'definitionHash', v_receipt.definition_hash,
        'stepId', v_receipt.step_id,
        'completionOrdinal', v_receipt.completion_ordinal,
        'expectedRunVersion', v_receipt.expected_run_version,
        'resultingRunVersion', v_receipt.resulting_run_version,
        'result', v_receipt.result,
        'resultHash', v_receipt.result_hash,
        'eventId', v_receipt.transition_event_id,
        'actor', v_receipt.actor
      )
    );
  else
  ...
  insert into workspace.work_events (
    ...
    v_event.id, p_workflow_run_id, p_workflow_run_id, p_event_kind, p_actor, p_event_summary,
    coalesce(p_event_payload, '{}'::jsonb) || jsonb_build_object(
      'executionVersion', v_execution_version,
      'definitionHash', v_definition_hash,
      'approvalStepId', p_approval_step_id,
    ...
    p_idempotency_key, p_request_hash, v_next_version, p_side_step_id,
    p_assignment_id, v_runtime_session_id
  ) returning * into v_event;
  return v_result || jsonb_build_object('event', to_jsonb(v_event));
end;
$$;

revoke all on function workspace.settle_workflow_side_step(
  uuid, text, text, text, text, uuid, text, text, text, text, text, jsonb
) from public, authenticated;
grant execute on function workspace.settle_workflow_side_step(
  uuid, text, text, text, text, uuid, text, text, text, text, text, jsonb
) to anon, service_role;
