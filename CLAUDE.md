...
### Activity dashboard contract

Activity lives in Settings → Activity (`/settings?section=activity`); transitional `/agents?view=activity` links redirect there. Five default cards reuse the same bodies in Home and workspace dashboards: attention, progress, outcomes, usage and connections. Cards persist their period/target/workspace filters in the existing instrument pin config. Workspace widgets enforce their destination scope; connection configuration remains explicitly global.

Telemetry is read-only. Hermes and ACP live conversations contribute state and pending decisions. Workflow outcomes count each durable run once and keep Deferred distinct from Cancelled. Hermes history groups session lifetime cost by start date and coalesces missing costs to zero: zero aggregates are unknown, positive reported and estimated subtotals stay separate. ACP cumulative session cost/context are optional, never added into historical period totals. Unknown workspace attribution is excluded, not replaced with global data. See `design/features/activity-dashboard/SPEC.md` for source/coverage boundaries.

### Settings capability ownership

Settings → Plugins is exclusively for IntelliZen Plugin SDK extensions. CLI plugins, skills, commands and MCP servers share Settings → Capabilities. Session capability choices are stored in the app's local `cli-capabilities.json`; native adapter startup applies supported Codex/Claude overrides without writing CLI configuration files. Existing chats retain their current selection until reconnected. Hermes profile controls explicitly retain their shared profile scope. Coverage and verification: `design/features/hermes-settings-parity/CLI-INVENTORY-VERIFICATION.md`.
