...
Activity lives in Agents → Activity (`/agents?view=activity`); old Settings Activity links redirect there. Five default cards reuse the same bodies in Home and workspace dashboards: attention, progress, outcomes, usage and connections. Cards persist their period/target/workspace filters in the existing instrument pin config. Workspace widgets enforce their destination scope; connection configuration remains explicitly global.
