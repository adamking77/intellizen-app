...
export function PluginErrorBox({ name, error }: { name: string; error: string }) {
  return (
    ...
    >
      <p className="font-ui text-[length:var(--t-ui)] leading-normal text-[var(--danger)]">Plugin “{name}” failed.</p>
      <p className="mt-0.5 break-words font-mono text-[length:var(--t-section)] leading-normal text-[var(--text-muted)]">{error}</p>
    </div>
  );
}
