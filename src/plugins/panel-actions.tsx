    ...
    <div className="flex shrink-0 flex-wrap gap-1.5 px-0.5 pb-1" aria-label="Plugin actions">
      {actions.map((action) => (
        <button
          ...
          className="rounded-[var(--r-pill)] border border-[var(--border)] bg-[var(--surface-wash)] px-2.5 py-1 font-ui text-[var(--t-section)] text-[var(--text)] transition-colors hover:border-[var(--border-strong)] hover:bg-[var(--hover)]"
        >
          {action.label}
        </button>
      ))}
