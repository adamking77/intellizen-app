    ...
    <div className={className ?? "grid gap-2"}>
      <div className="grid grid-cols-2 gap-2">
        <label className="grid gap-1.5">
          ...
          <select
            className="h-[var(--h-ctl)] rounded-[var(--r-ctl)] border border-[var(--border)] bg-[var(--base)] px-2.5 font-ui text-[length:var(--t-meta)] text-[var(--text)]  focus:outline-none"
            value={value.entity}
...
export function TaxonomySummary({ taxonomy }: { taxonomy?: TaxonomyMetadata | null }) {
  return (
    <span className="font-ui text-[length:var(--t-section)] text-[var(--overlay-1)]">
