...
describe("GraphEmbeds", () => {
  ...
  it("keeps an embedded graph in the document's authored position", async () => {
    graph.listGraphNodes.mockResolvedValue([
      { id: 1, project_id: 7, node_id: "one", label: "One", entity_type: "person", position_x: 0, position_y: 0, created_at: "", updated_at: "" },
    ]);
    graph.listGraphEdges.mockResolvedValue([]);
    ...
    await act(async () => {
      root.render(
        <MemoryRouter>
          <QueryClientProvider client={query}>
            <MarkdownBody content={'# Evidence\n\n```graph {"id":"7","mode":"construct"}\n```\n\nConclusion'} />
          </QueryClientProvider>
        </MemoryRouter>,
      );
    });

    const body = element.querySelector(".intelizen-doc-markdown");
    expect(body?.children[0]?.textContent).toBe("Evidence");
    expect(body?.children[1]?.tagName).toBe("FIGURE");
    expect(body?.children[2]?.textContent).toBe("Conclusion");
  });
});
