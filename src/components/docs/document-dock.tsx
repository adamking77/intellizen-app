...
export function DocumentDock({ mode, onModeChange, positionLabel, readingFocus, onReadingFocus, proposalCount, readOnly = false, decisionBusy = false }: {
  ...
  return <footer aria-label="Document controls" className="flex min-h-12 shrink-0 flex-wrap items-center gap-1 border-t border-[var(--line)] bg-[var(--ground)] px-3 py-1">
    <Control size="sm" variant={mode === "read" ? "selected" : "quiet"} disabled={decisionBusy} aria-pressed={mode === "read"} onClick={() => onModeChange("read")}>Reading</Control>
    <Control size="sm" variant={mode === "edit" ? "selected" : "quiet"} disabled={readOnly || decisionBusy} aria-pressed={mode === "edit"} onClick={() => onModeChange("edit")}>Edit</Control>
    ...
    <Control size="sm" variant={readingFocus ? "selected" : "quiet"} aria-pressed={readingFocus} onClick={onReadingFocus} className={cn(readingFocus && "text-[var(--text)]")}>Reading focus</Control>
  </footer>;
}
