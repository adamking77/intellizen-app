  ...
  function folder(node: DocumentFolder, level = 0) {
    ...
    return <li key={node.path}>
      <button type="button" aria-expanded={open} title={node.path} onClick={() => toggle(node.path)} onKeyDown={event => {
        ...
        <span className="min-w-0 flex-1 truncate text-left">{node.name}</span><span className="text-[length:var(--t-count)] text-[var(--text-muted)]">{node.count || ''}</span>
      </button>
      {open ? <ul aria-label={`${node.path} contents`}>{node.folders.map(child => folder(child, level + 1))}{node.records.map(record => <li key={record.id}>{row(record, level + 1)}</li>)}{!node.count && !node.folders.length ? <li className="px-6 py-2 text-[length:var(--t-meta)] text-[var(--text-muted)]">Empty folder</li> : null}</ul> : null}
    </li>;
  }
  ...
  return <aside className="relative flex min-h-0 shrink-0 flex-col border-r border-[var(--line)] bg-[var(--ground)]" style={{ width }} aria-label="Documents">
    <div className="grid gap-2 p-3">
      ...
      {newFolder !== null ? <form className="grid gap-2" onSubmit={async event => { event.preventDefault(); setFolderBusy(true); try { await onCreateFolder(newFolder); setNewFolder(null); setExpanded(current => [...new Set([...current, activeFolder])]); } catch (error) { setFolderError(String(error)); } finally { setFolderBusy(false); } }}>
        <span className="truncate text-[length:var(--t-meta)] text-[var(--text-muted)]" title={activeFolder || 'vault'}>New folder in {activeFolder || 'vault'}</span><Input autoFocus aria-label="Folder name" value={newFolder} onChange={event => setNewFolder(event.target.value)} onKeyDown={event => { if (event.key === 'Escape') setNewFolder(null); }} /><div className="flex gap-2"><Control type="submit" loading={folderBusy}>Create</Control><Control onClick={() => setNewFolder(null)}>Cancel</Control></div>{folderError ? <p role="alert" className="text-[length:var(--t-meta)] text-[var(--bad)]">{folderError}</p> : null}
      </form> : null}
    </div>
    <div className="min-h-0 flex-1 overflow-y-auto px-1.5 pb-3">
      {workspaceError ? <p role="alert" className="px-2 py-2 text-[length:var(--t-meta)] text-[var(--bad)]">Saved workspace documents could not be refreshed. <Control onClick={onRefresh}>Retry</Control></p> : null}
      {loadingVault ? <p role="status" className="px-2 py-2 text-[length:var(--t-meta)] text-[var(--text-muted)]">Reading vault folders…</p> : null}
      {vaultError || inventory?.errors.length ? <div role="alert" className="px-2 py-2 text-[length:var(--t-meta)] text-[var(--bad)]">{vaultError ? 'Vault folders could not be read. Saved documents remain available.' : `${inventory?.errors.length} folders could not be read. This list is incomplete.`}<Control variant="quiet" onClick={onRefresh}>Retry</Control></div> : null}
      {view === 'folders' ? <>
      ...
      {!filtered.length && !tree.folders.length && !loadingVault ? <p className="px-2 py-3 text-[length:var(--t-meta)] text-[var(--text-muted)]">{searchQuery ? 'No documents match this search.' : view === 'favorites' ? 'Star a document to add it to Favorites.' : 'No documents in this view.'}</p> : null}
    </div>
    <p className="truncate px-3 pb-2 text-[length:var(--t-meta)] text-[var(--text-muted)]" title={activeFolder || 'journal'}>New notes → {activeFolder || 'journal'}</p>
  </aside>;
}
