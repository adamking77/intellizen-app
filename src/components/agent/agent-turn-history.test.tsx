...
describe("historical tool outcome presentation", () => {
  it("keeps historical output neutral while retaining tool failure text for inspection", () => {
    ...
    const html = renderToStaticMarkup(<AgentTurn message={transcript.messages[0]} profile={null} />);
  ...
  it("keeps a live tool without a result in the running state", () => {
    const html = renderToStaticMarkup(<AgentTurn message={{ id: "live", from: "fiona", text: "", tools: [{ id: "tool", name: "read_file", title: "read_file" }] }} profile={null} />);
