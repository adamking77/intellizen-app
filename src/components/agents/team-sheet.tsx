...
export function TeamSheet({
  ...
  return (
    <AppDialog
      ...
      bodyClassName="p-0"
    >
            <div className="grid gap-3 px-[17px] pb-3">
              ...
              {agents.length >= TEAM_SEARCH_THRESHOLD ? (
                <label className="grid gap-1">
                  <span className={LABEL}>Search members</span>
                  <input className={VALUE} value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search agents…" aria-label="Search agents" />
                </label>
              ) : null}
            </div>
