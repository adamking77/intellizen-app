...
import { getRecordTitle, STATUS_OPTIONS } from "@/lib/database-core";
...
export function InlineEditor({ record, field, database, catalog, onSave, onCancel }: InlineEditorProps) {
  ...
  switch (field.type) {
    ...
    case "status":
      return (
        <InlinePillPicker
          options={field.options?.length ? field.options : [...STATUS_OPTIONS]}
          value={(currentValue as string) ?? null}
          getColor={(option) => resolveStatusColor(option, field)}
          groupStatus
          onSave={onSave}
          onCancel={onCancel}
        />
...
function InlineRelationEditor({
  ...
  const targetDb = useMemo(() => {
    ...
    if (!targetId || targetId === database.id) {
      return { id: database.id, schema: database.schema, records: database.records, headerFieldIds: database.headerFieldIds };
    }
    const found = catalog.find((entry) => entry.id === targetId);
    if (found) {
      return { id: found.id, schema: found.schema, records: found.records, headerFieldIds: found.headerFieldIds };
    }
    return { id: database.id, schema: database.schema, records: database.records, headerFieldIds: database.headerFieldIds };
  }, [field.relation?.targetDatabaseId, database.id, database.schema, database.records, database.headerFieldIds, catalog]);
  ...
  const candidates = useMemo(() => {
    ...
    return targetDb.records
      ...
      .filter((candidate) => {
        ...
        return getRecordTitle(candidate, targetDb).toLowerCase().includes(lowered);
      });
