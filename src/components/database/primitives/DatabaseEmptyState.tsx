  ...
  return (
    <div className="flex h-full items-center justify-center p-6">
      <div className="max-w-md rounded-2xl border border-dashed border-[var(--border)] bg-[var(--mantle)] px-6 py-10 text-center">
        ...
        {description ? <div className="mt-2 text-[13px] leading-6 text-[var(--subtext-0)]">{description}</div> : null}
        {action ? (
          <div className="mt-4 flex justify-center">
            <DatabaseButton onClick={action.onClick}>{action.label}</DatabaseButton>
          </div>
        ) : null}
      </div>
    </div>
