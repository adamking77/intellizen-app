...
export function WorkflowRunPulse({ events, pendingQuestion }: { events: WorkEventItem[]; pendingQuestion: boolean }) {
  ...
  if (!current) return <p className="mt-2 text-[var(--t-meta)] text-[var(--text-muted)]">No durable run receipts were recorded.</p>;
  return <div className="mt-2 space-y-3">
    <p className="text-[var(--t-count)] text-[var(--text-muted)]">Receipt order · {index + 1} of {ordered.length}{events.length >= 500 ? " · Latest 500 receipts; earlier history is outside this view." : ""}</p>
    <div className="space-y-2" aria-label="Agent receipt traces">
      {traces.map((trace) => <div key={trace.agent}>
        <p className="break-words text-[var(--t-meta)] text-[var(--text)]">{trace.agent}</p>
      ...
      {!traces.length ? <p className="text-[var(--t-meta)] text-[var(--text-muted)]">No agent assignment receipts in this part of the run.</p> : null}
    ...
    {ordered.length > 1 ? <div className="flex flex-wrap items-center gap-2">
      <label className="flex min-w-0 flex-1 items-center gap-2 text-[var(--t-count)] text-[var(--text-muted)]">Replay receipt
    ...
    <p className="break-words text-[var(--t-meta)] text-[var(--text-muted)]">{current.run_version != null ? `v${current.run_version} · ` : ""}{current.summary || current.event_kind}</p>
    <details><summary className="text-[var(--t-meta)]">Receipts through this point</summary>
      <ol className="mt-2 max-h-48 space-y-2 overflow-y-auto text-[var(--t-meta)] text-[var(--text-muted)]">{visible.map((event) => <li key={event.id} className="break-words">{event.run_version != null ? `v${event.run_version} · ` : ""}{event.summary || event.event_kind}</li>)}</ol>
    </details>
  </div>;
}
