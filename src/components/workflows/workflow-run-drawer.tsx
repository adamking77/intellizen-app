...
export function WorkflowRunDrawer({ runId, item, onClose }: { runId: string; item: WorkflowCatalogItem | null; onClose: () => void }) {
  ...
  return <Drawer open onClose={onClose} label={run?.name ?? "Workflow run"} className={`${fullPage ? "inset-2 w-auto" : "w-[min(620px,calc(100%-16px))]"} p-5`}>
    ...
    {query.isLoading ? <Skeleton lines={8} /> : query.error ? <p role="alert" className="mt-4 text-[var(--danger)]">Could not load this run. <Control onClick={() => void query.refetch()}>Retry</Control></p> : run ? <div className="mt-4 space-y-5">
      ...
      {fullPage ? <section aria-label="Run timeline" className="border-y border-[var(--border)] py-3">{eventsQuery.isLoading ? <p className="text-[var(--t-meta)] text-[var(--text-muted)]">Loading durable receipts…</p> : eventsQuery.error ? <p role="alert" className="text-[var(--t-meta)] text-[var(--danger)]">Could not load run receipts. <Control size="sm" onClick={() => void eventsQuery.refetch()}>Retry</Control></p> : <WorkflowRunPulse key={runId} events={eventsQuery.data ?? []} pendingQuestion={Boolean(approvalQuestion)} />}</section> : null}
      ...
      {approvalNotice ? <p role="status" className="text-[var(--t-meta)] text-[var(--warning)]">{approvalNotice}</p> : null}
      ...
      {waitingError ? <p role="alert" className="text-[var(--t-meta)] text-[var(--danger)]">{waitingError}</p> : null}
