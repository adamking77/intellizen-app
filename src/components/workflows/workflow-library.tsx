    ...
    {visible.length ? <div className={view === "cards" ? "grid grid-cols-[repeat(auto-fit,minmax(min(100%,280px),1fr))] gap-3" : "divide-y divide-[var(--row-line)]"}>
      {visible.map((item) => {
        ...
        return <button key={item.workflow.id || item.workflow.workflow_id} type="button" onClick={() => onOpen(item)} aria-label={`Edit ${item.workflow.name}`} className={view === "cards" ? "flex min-w-0 flex-col rounded-[var(--r-surface)] bg-[var(--surface)] p-4 text-left transition-colors hover:bg-[var(--hover)]" : "grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-x-4 gap-y-1 px-4 py-3 text-left transition-colors hover:bg-[var(--hover)]"}>
          <span className="block text-[length:var(--t-body)] font-medium leading-snug text-[var(--text)]">{item.workflow.name}</span>
          <span className={`${view === "cards" ? "mt-2 line-clamp-2" : "col-span-1 line-clamp-1"} text-[length:var(--t-meta)] leading-relaxed text-[var(--text-muted)]`}>{item.workflow.expected_output || "Review the definition to finish this workflow."}</span>
          <span aria-label="Workflow steps" className={`${view === "cards" ? "mt-2" : "col-span-1"} font-mono text-[length:var(--t-count)] text-[var(--text-dim)]`}>{definition ? `${definition.steps.length} ${definition.steps.length === 1 ? "step" : "steps"} · ${roleCount} ${roleCount === 1 ? "role" : "roles"}` : "Definition needs review"}</span>
          <span className={`${view === "cards" ? "mt-auto pt-4" : "row-span-2 col-start-2 row-start-1 self-center"} flex flex-wrap items-center justify-between gap-x-3 gap-y-1 text-[length:var(--t-meta)]`}><span className="text-[var(--text-muted)]">{owner}</span><span className="text-[var(--text-muted)]">{state}</span></span>
          {item.blockers[0] ? <span className={`${view === "cards" ? "mt-2" : "col-span-2"} text-[length:var(--t-meta)] leading-relaxed text-[var(--text-muted)]`}>{item.blockers[0].message}{item.blockers.length > 1 ? ` (${item.blockers.length - 1} more ${item.blockers.length === 2 ? "issue" : "issues"})` : ""}</span> : null}
        </button>;
      })}
