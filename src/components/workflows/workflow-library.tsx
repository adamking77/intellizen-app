...
export function WorkflowLibrary({ items, onOpen, onCreate, onDraftWithAgent }: {
  ...
  return <section aria-label="Workflow library" className="min-w-0">
    ...
    {visible.length ? <div className={view === "cards" ? "grid grid-cols-[repeat(auto-fit,minmax(min(100%,280px),1fr))] gap-3" : "divide-y divide-[var(--border)] rounded-[var(--r-ctl)] bg-[var(--raised)]"}>
      {visible.map((item) => {
        ...
        return <button key={item.workflow.id || item.workflow.workflow_id} type="button" onClick={() => onOpen(item)} aria-label={`Edit ${item.workflow.name}`} className={view === "cards" ? "flex min-w-0 flex-col rounded-[var(--r-ctl)] bg-[var(--raised)] p-4 text-left transition-colors hover:bg-[var(--hover)]" : "grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-x-4 gap-y-1 px-4 py-3 text-left transition-colors hover:bg-[var(--hover)]"}>
          ...
          <p aria-label="Workflow steps" className={`${view === "cards" ? "mt-2" : "col-span-1"} font-mono text-[var(--t-count)] text-[var(--text-dim)]`}>{definition ? `${definition.steps.length} ${definition.steps.length === 1 ? "step" : "steps"} · ${roleCount} ${roleCount === 1 ? "role" : "roles"}` : "Definition needs review"}</p>
    ...
    <footer className="mt-5 flex flex-wrap items-center justify-between gap-2 border-t border-[var(--border)] pt-2" aria-label="Workflow library controls">
      <div className="flex shrink-0 gap-1"><Control size="sm" variant={view === "cards" ? "primary" : "quiet"} onClick={() => setView("cards")}><LayoutGrid aria-hidden className="h-3.5 w-3.5" />Cards</Control><Control size="sm" variant={view === "list" ? "primary" : "quiet"} onClick={() => setView("list")}><List aria-hidden className="h-3.5 w-3.5" />List</Control></div>
