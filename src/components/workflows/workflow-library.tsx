...
export function WorkflowLibrary({ items, onOpen, onCreate, onDraftWithAgent }: {
  ...
  return <section aria-label="Workflow library" className="min-w-0">
    ...
    {visible.length ? <div className={view === "cards" ? "grid grid-cols-[repeat(auto-fit,minmax(min(100%,280px),1fr))] gap-3" : "divide-y divide-[var(--border)] rounded-[var(--r-ctl)] bg-[var(--raised)]"}>
      {visible.map((item) => {
        ...
        return <button key={item.workflow.id || item.workflow.workflow_id} type="button" onClick={() => onOpen(item)} aria-label={`Edit ${item.workflow.name}`} className={view === "cards" ? "flex min-w-0 flex-col rounded-[var(--r-ctl)] bg-[var(--raised)] p-4 text-left transition-colors hover:bg-[var(--hover)]" : "grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-x-4 gap-y-1 px-4 py-3 text-left transition-colors hover:bg-[var(--hover)]"}>
          <h2 className="text-[var(--t-body)] font-medium leading-snug text-[var(--text)]">{item.workflow.name}</h2>
          <p className={`${view === "cards" ? "mt-2 line-clamp-2" : "col-span-1 line-clamp-1"} text-[var(--t-meta)] leading-relaxed text-[var(--text-muted)]`}>{item.workflow.expected_output || "Review the definition to finish this workflow."}</p>
          <p aria-label="Workflow steps" className={`${view === "cards" ? "mt-2" : "col-span-1"} font-mono text-[var(--t-count)] text-[var(--text-dim)]`}>{definition ? `${definition.steps.length} steps · ${roleCount} roles` : "Definition needs review"}</p>
          <div className={`${view === "cards" ? "mt-auto pt-4" : "row-span-2 col-start-2 row-start-1 self-center"} flex flex-wrap items-center justify-between gap-x-3 gap-y-1 text-[var(--t-meta)]`}><span className="text-[var(--text-muted)]">{owner}</span><span className={item.blockers.length ? "text-[var(--warning)]" : "text-[var(--text-muted)]"}>{state}</span></div>
          {item.blockers[0] ? <p className={`${view === "cards" ? "mt-2" : "col-span-2"} text-[var(--t-meta)] leading-relaxed text-[var(--warning)]`}>{item.blockers[0].message}{item.blockers.length > 1 ? ` (${item.blockers.length - 1} more ${item.blockers.length === 2 ? "issue" : "issues"})` : ""}</p> : null}
        </button>;
      })}
    ...
    <footer className="mt-5 flex flex-wrap items-center gap-1 border-t border-[var(--border)] pt-2" aria-label="Workflow library controls"><Control size="sm" variant={view === "cards" ? "primary" : "quiet"} onClick={() => setView("cards")}><LayoutGrid aria-hidden className="h-3.5 w-3.5" />Cards</Control><Control size="sm" variant={view === "list" ? "primary" : "quiet"} onClick={() => setView("list")}><List aria-hidden className="h-3.5 w-3.5" />List</Control><Control size="sm" onClick={onDraftWithAgent}>Draft with an agent</Control><Control size="sm" className="ml-auto" onClick={onCreate}><Plus aria-hidden className="h-3.5 w-3.5" />New workflow</Control></footer>
  </section>;
}
