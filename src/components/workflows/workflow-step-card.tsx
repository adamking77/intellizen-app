...
export function WorkflowStepCard({ step, index, definition, selected, expanded = selected, roleTargets, onSelect, onChange, onAskRole, actions, typeEditor, autoFocusTitle = false }: {
  ...
  return <Surface data-workflow-step={step.id} data-selected={selected || undefined} className={`px-[11px] py-[9px] ${selected ? "bg-[var(--selected)] hover:bg-[var(--selected-hover)]" : ""}`}>
    <div className={`flex items-start gap-2${editing ? " nodrag nopan" : ""}`}>
      {editing ? <>
        <Input data-workflow-field="title" autoFocus={autoFocusTitle} aria-label={`Title for step ${index + 1}`} value={step.title} onChange={(event) => onChange({ ...step, title: event.target.value })} className="min-w-0 flex-1 font-medium" />
        ...
        <span className="block break-words text-[length:var(--t-ui)] font-medium">{step.title}</span>
    ...
    {editing ? <div className="nodrag nopan mt-2 space-y-3 cursor-auto">
      ...
      {step.kind === "role-assign" ? <>
        ...
        <label className={labelClass}>Task<Textarea data-workflow-field="instructions" aria-label={`Instructions for ${step.title}`} value={step.instructions} onChange={(event) => onChange({ ...step, instructions: event.target.value })} rows={2} className="mt-1 h-14 min-h-0 [field-sizing:fixed] text-[length:var(--t-meta)] leading-5" /></label>
      </> : null}
      {step.kind === "condition" ? <>
      ...
      {step.kind === "approval" ? <><label className={labelClass}>Gate<Input data-workflow-field="gate" aria-label={`Approval gate for ${step.title}`} value={step.gate} onChange={(event) => onChange({ ...step, gate: event.target.value })} className="mt-1" /></label><section className="space-y-2 border-t border-[var(--border)] pt-3"><div><p className="text-[12px] text-[var(--text)]">Meanwhile</p><p className="mt-0.5 text-[length:var(--t-count)] text-[var(--text-muted)]">Independent, read-only side work only.</p></div>{meanwhile.length ? meanwhile.map((candidate) => { const checked = step.meanwhile?.includes(candidate.id) ?? false; const blocker = checked ? meanwhileBindingBlocker(candidate, roleTargets) : null; return <div key={candidate.id}><label className="flex items-start gap-2 text-[12px]"><Checkbox aria-label={`Run ${candidate.title} meanwhile`} checked={checked} onCheckedChange={(enabled) => onChange({ ...step, meanwhile: enabled ? [...(step.meanwhile ?? []), candidate.id] : step.meanwhile?.filter((id) => id !== candidate.id), reminder: "never" })} /><span><span className="block text-[var(--text)]">{candidate.title}</span><span className="block font-mono text-[length:var(--t-count)] text-[var(--text-muted)]">{candidate.id}</span></span></label>{blocker ? <p role="status" className="mt-1 text-[length:var(--t-count)] text-[var(--warning)]">{blocker}</p> : null}</div>; }) : <p className="text-[length:var(--t-count)] text-[var(--text-muted)]">No independent side step is eligible.</p>}<p className="text-[length:var(--t-count)] text-[var(--text-muted)]">Reminder: Never</p></section></> : null}
