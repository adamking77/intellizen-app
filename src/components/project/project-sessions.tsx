          ...
          {sessions.map((session) => (
            ...
            >
              <div className="grid min-w-0 gap-1">
                  <p className="truncate font-ui text-[length:var(--t-meta)] font-medium text-[var(--text)]">{session.title}</p>
          ...
          {transcriptOnly && selected ? (
            <header className="mx-auto mb-4 flex max-w-3xl items-end justify-between gap-4 border-b border-[var(--border-subtle)] pb-3">
              <div className="min-w-0">
                ...
                <h2 className="mt-1 truncate text-[length:var(--t-title)] text-[var(--text)]">{selected.title}</h2>
                <div className="mt-2"><Identity name={selected.profile} runtime="hermes" /></div>
              </div>
              <div className="shrink-0 text-right font-mono text-[length:var(--t-count)] text-[var(--text-muted)]">
            ...
            <div className="mx-auto max-w-3xl divide-y divide-[var(--border-subtle)]">
              {(transcript.data ?? []).map((message) => (
                <article key={message.id} className={cn("py-4", message.role === "user" && "rounded-[var(--r-ctl)] bg-[var(--raised)] px-4")}>
                  {message.role === "tool" ? <details><summary className="cursor-pointer"><ToolRow tool={message.name ?? "tool"} detail={message.text} state="recorded" /></summary><pre className="mt-2 whitespace-pre-wrap break-words text-[length:var(--t-meta)] text-[var(--text-muted)]">{message.text}</pre></details> : message.role === "system" ? <Receipt verb="recorded" object={message.text} /> : (
              ...
              {transcriptOnly && selected ? <section aria-label="Session receipts" className="mx-auto grid max-w-3xl gap-2 py-4">
                ...
                {events.isLoading ? <p role="status" className="text-[length:var(--t-meta)] text-[var(--text-muted)]">Loading session receipts…</p> : null}
                {events.error ? <div role="alert" className="text-[length:var(--t-meta)] text-[var(--danger)]">Session receipts could not be refreshed. <Control size="sm" variant="quiet" onClick={() => void events.refetch()}>Retry receipts</Control></div> : null}
                {receipts.map((event) => <SessionReceipt key={event.id} event={event} />)}
                {!events.isLoading && !events.error && !receipts.length ? <p className="text-[length:var(--t-meta)] text-[var(--text-muted)]">No receipts are linked to this session.</p> : null}
                {(events.data?.length ?? 0) >= 500 ? <p className="text-[length:var(--t-count)] text-[var(--text-muted)]">Showing the latest 500 linked receipts.</p> : null}
              </section> : null}
        </div>
      </QueryState>
      <Drawer open={drawerSession != null} onClose={closeDrawer} label={drawerSession?.title ?? "Session details"}>
        {drawerSession ? <div className="grid gap-5 p-4"><div><div className="text-[length:var(--t-count)] uppercase tracking-[0.14em] text-[var(--text-muted)]">Session</div><h2 className="mt-1 text-[length:var(--t-title)] text-[var(--text)]">{drawerSession.title}</h2></div><Identity name={drawerSession.profile} runtime="hermes" /><Receipt className="ml-0" verb="linked" object={`${drawerSession.messageCount} messages · ${formatTime(drawerSession.lastActive)}`} /><Control variant="primary" onClick={() => { setSelection({ scope: selectionScope, key: projectSessionKey(drawerSession) }); setDrawerKey(null); }}>Open session</Control></div> : null}
      </Drawer>
    </div>
  );
}
...
function SessionReceipt({ event }: { event: WorkEventItem }) {
  ...
  return <div className="min-w-0">
    ...
    <div className="ml-3.5 flex flex-wrap items-center gap-3 text-[length:var(--t-count)]">
