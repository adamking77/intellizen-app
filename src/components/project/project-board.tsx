          ...
          {(boardData.data ?? []).map(({ board, columns }) => (
            <section key={board.slug} aria-label={board.name}>
              <div className="mb-2 flex items-center justify-between gap-3">
                <h2 className="font-ui text-[13px] font-semibold text-[var(--text)]">{board.name}</h2>
                <span className="font-mono text-[10px] text-[var(--overlay-1)]">{board.total} cards</span>
              </div>
                ...
                {columns.map((column) => (
                  <div key={column.name} className="w-56 shrink-0">
                    <div className="mb-2 flex items-center justify-between border-b border-[var(--border-subtle)] pb-2">
                      ...
                      <span className="rounded-full border border-[var(--border)] px-1.5 py-0.5 font-mono text-[9px] text-[var(--overlay-1)]">
                        {column.cards.length}
                      </span>
                    </div>
                    <div className="space-y-2">
                      {column.cards.map((card) => (
                        <article key={card.id} className="rounded-xl border border-[var(--border)] bg-[var(--surface-wash)] p-3">
                          <p className="font-ui text-[12px] font-medium leading-5 text-[var(--text)]">{card.title}</p>
                          {card.latestSummary ? (
                            <p className="mt-1 line-clamp-2 font-ui text-[11px] leading-4 text-[var(--subtext-0)]">{card.latestSummary}</p>
                          ) : null}
