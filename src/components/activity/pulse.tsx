...
/** A quiet, data-shaped trace. It deliberately does not run a decorative loop. */
export function Pulse({ traces = [], questions = [], state = "known", className }: { traces?: PulseTrace[]; questions?: string[]; state?: "known" | "loading" | "unknown"; className?: string }) {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={label}
      className={cn("h-14 w-full max-w-[760px] overflow-visible", className)}
    >
      {paths.map((trace, index) => {
        const y = 28 + (index - (paths.length - 1) / 2) * Math.min(5, 40 / Math.max(1, paths.length - 1));
        const wave = 5 + index * 2;
        return <path key={trace.id} d={`M0 ${y} C80 ${y - wave}, 100 ${y - wave}, 160 ${y} S240 ${y + wave}, 320 ${y} S400 ${y - wave}, 480 ${y} S560 ${y + wave}, ${width} ${y}`} fill="none" stroke={trace.color || "var(--text-dim)"} strokeWidth="1.15" opacity="0.8" />;
      })}
