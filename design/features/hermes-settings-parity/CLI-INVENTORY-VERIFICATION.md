...
## Full visual follow-up — passed

The fresh independent pass authorized by Adam completes the previously interrupted provider-selection, Hermes-scope and narrow/200% checks. Native review verified all-provider and Codex/Hermes filtering, search/no-match recovery, grouped CLI inventory, visible configured switch states, shared Hermes profile scope, full expandable descriptions, SDK-only Plugins and a truthful unsupported-provider deep link. Macchiato and Latte, narrow content panes and 200% zoom passed after the Settings drawer and description repairs. Drawer dismissal returns focus to its exact opener, and responsive navigation preserves the saved rail preference.

The final full frontend suite passes 708 tests with one existing skip; check, safe production build and bundle-secret scan pass. No native implementation changed in this follow-up. The reviewer restored original appearance, pane sizes, filters and empty draft. Report: `design/features/activity-dashboard/evidence/FULL-VISUAL-VERIFICATION.md`; shared receipt: `design/features/activity-dashboard/VERIFICATION.md`. Private screenshots remain local. Real capability switches and connections were not changed; the switch-enforcement evidence and live-provider limitation above remain applicable.
